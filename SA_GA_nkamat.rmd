...
set.seed(1234)
fn.call <<- 0
out.GenSA <- GenSA(fn = obj, lower = rep(0, 2), upper = rep(10, 2), control = list(smooth = FALSE, max.call = 3000))
fn.call.GenSA <- fn.call
out.GenSA$value
out.GenSA$counts
cat("GenSA call functions", fn.call.GenSA, "times.\n")
...
gap <- ga(type = "real-valued", fitness=obj, lower=rep(0,2), upper=rep(10,2), popSize = 50, maxiter = 100)
summary(gap)
plot(gap)
```
```{r}
